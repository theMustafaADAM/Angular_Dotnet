...
#pragma warning disable CS8604 // Possible null reference argument.
    ...
    public class AuthSvc : IAuthSvc
    {
        ...
        private string unProtectodToken = string.Empty;
...
#pragma warning disable CS8618 // Non-nullable field must contain a non-null value when exiting constructor. Consider declaring as nullable.
        ...
        public async Task<TokenResponseModel> Auth(LoginViewModel model)
        {
            ...
            {
                Log.Error("An error occurred while seeding the Authentication : {Error} {StackTrace} {InnerException} {Source}",
        ...
        public async Task<bool> LogoutUserAsync()
        {
            ...
            {
                Log.Error("An error occurred while Loging out User : {Error} {StackTrace} {InnerException} {Source}",
                    ex.Message, ex.StackTrace, ex.InnerException, ex.Source);
            }
            _cookieSvc.DeleteAllCookies(cookiesToDelete);
            return false;
        ...
        public async Task<TokenResponseModel> GenerateNewToken()
        {
            ...
            {
                Log.Error("An error occurred at Generate New Token :  {Error} {StackTrace} {InnerException} {Source}",
                    ex.Message, ex.StackTrace, ex.InnerException, ex.Source);
            }
        ...
        private async Task<TokenResponseModel> GenerateNewToken(TokenRequestModel model)
        {
            try
            {
                ...
                if (user != null && await _userManager.CheckPasswordAsync(user, model.Password) && user.UserRole != "Administrator")
                {
                    ...
                    _cookieSvc.SetCookie("loginStatus", "1", Convert.ToInt32(refreshTokenExpireTime), false, false);
                    _cookieSvc.SetCookie("username", user.UserName, Convert.ToInt32(refreshTokenExpireTime), false, false);
                    _cookieSvc.SetCookie("userRole", value: user.UserRole, Convert.ToInt32(refreshTokenExpireTime), false, false);
                    _cookieSvc.SetCookie("user_id", accessToken.UserID, Convert.ToInt32(refreshTokenExpireTime));
                    return accessToken;
        ...
        private async Task<TokenResponseModel> CreateAccessToken(ApplicationUser user)
        {
            ...
            var key = new SymmetricSecurityKey(Encoding.ASCII.GetBytes(s: _appSettings.Secret));
            ...
            try
            {
                ...
                var rt = await _db.Tokens.Where(t => t.UserID == user.Id).ToListAsync();
            ...
            {
                Log.Error("An error occurred while Create Access Token : {Error} {StackTrace} {InnerException} {Source}",
                    ex.Message, ex.StackTrace, ex.InnerException, ex.Source);
            }
            ...
            var encAuthToken = new TokenResponseModel
            {
                ...
                RefreshTokenExpiration = newRtoken.ExpireTime,
        ...
        private async Task<TokenResponseModel> RefreshToken(TokenRequestModel model)
        {
            try
            {
                if (_appSettings.AllowSiteWideTokenRefresh)
                {
                    ...
                    if (jwtValidationResult.IsValid && jwtValidationResult.Message == "Token Expired")
                    {
                        ...
                        var rt = _db.Tokens.FirstOrDefault(t => t.ClientID == _appSettings.ClientID && t.UserID == user.Id);
                        ...
                        if (rt.ExpireTime < DateTime.UtcNow)
                        {
            ...
            {
                Log.Error("An error occurred at Refreshing Token : {Error} {StackTrace} {InnerException} {Source}",
                    ex.Message, ex.StackTrace, ex.InnerException, ex.Source);
                ...
                if (!string.IsNullOrEmpty(authToken))
                {
                    ...
                    if (user != null)
                    {
                        ...
                        if (userOldToken != null)
                        {
                            ...
                            var unProtectedToken = protectorJwt.Unprotect(unprotectedTokenLayerOne);
            ...
            catch (Exception ex)
            {
                if (ex.GetType() == typeof(SecurityTokenExpiredException))
                {
                    if (_appSettings.AllowSiteWideTokenRefresh)
                    {
                        ...
                        validateToken = handler.ValidateToken(unProtectodToken, validationParameters, out var securityToken);
                ...
                Log.Error("An error occurred while Validating Authentication Token : {Error} {StackTrace} {InnerException} {Source}",
                    ex.Message, ex.StackTrace, ex.InnerException, ex.Source);
            }
            response.IsValid = false;
            response.Message = "Token Invalid";
        ...
        private async Task<TokenResponseModel> GenerateNewToken(ApplicationUser user, LoginViewModel model)
        {
            ...
            {
                Log.Error("An error occurred while Generating New Token : {Error} {StackTrace} {InnerException} {Source}",
                    ex.Message, ex.StackTrace, ex.InnerException, ex.Source);
            }
        ...
        private static TokenModel CreateRefreshToken(string clientId, string userId, int expireTime)
        {
            ...
            return new TokenModel()
            {
                ...
                ExpireTime = DateTime.UtcNow.AddMinutes(expireTime),
                EncryptionKeyRt = "",
                EncryptionKeyJwt = ""
            };
        }
