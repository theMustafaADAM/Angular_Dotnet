...
#pragma warning disable CS8600 // Converting null literal or possible null value to non-nullable type.
        ...
        public async Task<ProfileModel?> GetUserProfileByUsernameAsync(string username)
        {
            ...
            try
            {
                userProfile = new ProfileModel()
                {
                    ...
                    UserAddresses = new List<AddressModel>(
                        await _db.addresses.Where(x => x.UserID == user.Id).Select(n =>
                    new AddressModel()
                    {
                        UserID = n.UserID,
                        Line1 = n.Line1,
                    ...
                    Activities = new List<ActivityModel>(_db.Activities.Where(u => u.UserID == user.Id))
        ...
        public async Task<List<ActivityModel>?> GetUserActivity(string username)
        {
            ...
            try
            {
                ...
                userActivities = await _db.Activities.Where(x => x.UserID == user.Id)
                    .OrderByDescending(o => o.Date).Take(20).ToListAsync();
        ...
        public async Task<bool> CheckPasswordAsync(ProfileModel model, string password)
        {
            try
            {
                ...
                if (user.UserName != _cookieSvc.Get("username") || user.UserName != model.Username) return false;
        ...
        public async Task<bool> ChangePasswordAsync(ProfileModel model, string newpassword)
        {
            ...
            try
            {
                ...
                if (user != null)
                {
                    ...
                    activityModel.Type = "Password Changed Successfully";
        ...
        public async Task<bool> UpdateProfileAsync(IFormCollection formData)
        {
            ...
            if (user.UserName != _cookieSvc.Get("username") ||
                ...
                user.Email != formData["email"].ToString()) return false;
            try
            {
                ...
                /* IF Address exist we update them, IF not we add them */
                await InsetOrUpdateAddress(user.Id, "Shipping", formData["saddress1"], formData["saddress2"], formData["scountry"], formData["sstate"], formData["scity"], formData["spostalcode"], formData["sunit"]);
                await InsetOrUpdateAddress(user.Id, "Billing", formData["address1"], formData["address2"], formData["country"], formData["state"], formData["city"], formData["postalcode"], formData["unit"]);
        ...
        public async Task<bool> AddUserActivity(ActivityModel activityModel)
        {
            try
            {
                await _activitySvc.AddUserActivity(activityModel);
        ...
        {
            // First: Create empty array to store old file info
            var oldProfilePic = new string[1];

            // Will store path of old file to delete in empty array
            oldProfilePic[0] = Path.Combine(_env.WebRootPath + user.ProfilePic);

            // Then: Create the profile image path
            var profilePicPath = _env.WebRootPath + $"{Path.DirectorySeparatorChar}uploads{Path.DirectorySeparatorChar}user{Path.DirectorySeparatorChar}Profile{Path.DirectorySeparatorChar}";

            // if we have recived any files for update, then we update the file path after saving to server
            // otherwise we return the user without any changes
            if (formData.Files.Count <= 0) return user;

            var extension = Path.GetExtension(formData.Files[0].FileName);
            var filename = DateTime.Now.ToString("yymmssfff");
            var path = Path.Combine(profilePicPath, filename) + extension;
            var dbImagePath = Path.Combine($"{Path.DirectorySeparatorChar}uploads{Path.DirectorySeparatorChar}user{Path.DirectorySeparatorChar}Profile{Path.DirectorySeparatorChar}");

            user.ProfilePic = dbImagePath;

            // Coping New Files to the Server - Profile folder
        ...
        private async Task InsetOrUpdateAddress(string userID, string type, string line1, string line2, string country,
        ...
        public async Task<ResponseObject> ResetPassword(ResetPasswordViewModel model)
        {
            ...
            try
            {
                ...
                if (user == null)
                {
                    // Do Not reveal that the user was not exist or not confirmed
                    responseObject.Message = "Failed";
                    responseObject.IsValid = false;
                    responseObject.Data = null;
                    return responseObject;
            ...
            {
                Log.Error("An error occurred at Forgot Password:  {Error} * {StackTrace} * {InnerException} * {Source}",
                    ex.Message, ex.StackTrace, ex.InnerException, ex.Source);
            }
            responseObject.Message = "Failed";
            responseObject.IsValid = false;
