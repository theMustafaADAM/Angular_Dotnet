...
export class LoginComponent implements OnInit {
    ...
    onSubmit() {
        ...
        this.acct.login(userlogin.Username, userlogin.Password, userlogin.RememberMe).subscribe(
            (result) => {
                ...
                if (error.status == 401) {
                    if (error.error.loginError) {
                        ...
                        if (error.error.loginError == 'Account Locked') {
                            Swal.fire({
                                title: 'Your account is locked, please contact support.',
                                icon: 'error',
                                showClass: {
                        ...
                        if (error.error.loginError == 'Email Not Confirmed') {
                            Swal.fire({
                                title: 'Your Email is not Verified. Please Verify Email',
                                icon: 'error',
                                showClass: {
                                    popup: 'animate__animated animate__fadeInDown'
                                },
